...
- **Hot activation: DONE (2026-07-20)** — see `2026-07-20-hot-module-activation.md`: the engine initializes activated modules between sessions (`apply_pending_activations`, shared `DynModuleActivator` with the startup path), an api refresher rebuilds the ws api/dashboard and swaps iroh handlers per snapshot. Restart scheduling on `Activate` removed. E2E: `activated_module_runs_without_restart`, `offline_peer_catches_up_after_activation` (per-peer task groups in `FederationTest`), `activated_module_runs_after_restart` (startup-path parity). Solo-guardian fixes along the way: sessions complete without item flow, solo proposals approve at proposal time, solo DKG returns without busy looping.
...
- **Dashboard UI: DONE** — "Module Generation" card in the guardian dashboard (`fedimint-server-ui/src/dashboard/config_gen.rs`): generation table with lifecycle badges, propose dropdown (consensus version from module init, network from bitcoin backend), approve/activate/abort buttons gated on local state. Dashboard api methods share validation with the admin endpoints. Verified against real fedimintd processes via `scripts/dev/config-gen-ui-smoke.sh` (devimint dev-fed: propose + approve through all four dashboards → DKG → Ready for activation).
- **Next:** hot (no-restart) activation; genesis commit for existing federations; bootstrap-only setup; guardian recovery flow; UI error surfacing (action failures currently only log server-side). Phase 3 remainder (deterministic gen randomness) stays deferred — abort-and-retry makes it non-blocking.
