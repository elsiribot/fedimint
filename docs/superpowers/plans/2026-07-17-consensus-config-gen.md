...
- **Next:** Phase 5 (client config revisions: regenerate the served client config to include dynamic modules, wire the dormant client refresh path, threshold attestation), then hot activation, genesis commit, bootstrap-only setup, guardian recovery flow. Phase 3 remainder (deterministic gen randomness) stays deferred — abort-and-retry makes it non-blocking. devimint exercise deferred — the in-process e2e covers the full stack except process boundaries.
