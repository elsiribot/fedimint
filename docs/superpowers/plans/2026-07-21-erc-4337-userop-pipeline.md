...
- Artifacts (flattened layout `artifacts/<Name>.json`): `https://unpkg.com/@account-abstraction/contracts@0.7.0/artifacts/{EntryPoint,SimpleAccount,SimpleAccountFactory,TokenPaymaster,OracleHelper,LegacyTokenPaymaster}.json` (all HTTP 200). TetherToken: mainnet runtime bytecode via `eth_getCode` against a public RPC OR the committed Phase-4 approach; committed as hex fixture, `anvil_setCode` at `0xdAC17F958D2ee523a2206206994597C13D831ec7` + `anvil_setStorageAt` for balances/owner.

**Paymaster-economics scope decision (2026-07-21, autonomous — maintainer sign-off):** the "gas paid in USDT, federation ETH net-zero" layer needs the v0.7 sample `TokenPaymaster` (arbitrary ERC-20 + price oracle), whose devnet standup requires **mock oracles** (`OracleHelper`/`TestOracle2`) — non-trivial. Phase 8 formally owns paymaster/fee economics ("overcharge handling; paymaster-refusal → retry path"). Therefore **Phase 7's actual risk-retirement is the MPC-signed UserOp deploying + sweeping a counterfactual account on the REAL EntryPoint** — independent of the paymaster. Task 6 attempts the full `TokenPaymaster`+mock-oracle path; if that wiring exceeds devnet-reasonable effort within the acceptance, the Phase-7 gate proves deploy-and-sweep with **broadcaster-EOA gas** (`handleOps` beneficiary = broadcaster; federation *guardians* still spend 0 ETH — only the shared broadcaster fronts gas), and the gas-in-USDT/ETH-net-zero assertion moves to Phase 8. Vendor both paymasters + `OracleHelper` in Task 1 so the choice is unblocked at Task 6.
