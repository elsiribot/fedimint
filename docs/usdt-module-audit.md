...
| Withdrawal devimint e2e (real-chain) | **authored; sweep-in-devimint under investigation** | Deposit→claim runs green through real `fedimintd` + anvil (exact e-cash balance). The withdrawal extension (4337 deploy + EntryPoint prefunding + broadcaster override + withdraw/verify) is authored and compiles (WIP patch preserved), and gets through deploy→prefund→deposit→claim, but the automatic MPC-signed deploy-and-sweep UserOp does not fund the pool in the multi-process devimint env (the in-process anvil e2e signs+sweeps fine). Needs a `RUST_LOG=…,fedimint_usdt=debug` re-run to determine whether the multi-process MPC signing stalls or the sweep UserOp reverts on-chain. Not an architectural limitation. |
