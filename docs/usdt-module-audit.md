...
| Static `usdt_per_eth_e6 = $3000` price placeholder | **resolved** | Each guardian now reads a live Chainlink `AggregatorV3.latestRoundData()` (`eth_usd_price_feed`, default the canonical mainnet ETH/USD feed) into its `FeeVote`, staleness/sanity-guarded (`chainlink_eth_usd_to_usdt_per_eth_e6`); a bad/stale/unreachable read makes that guardian abstain rather than vote a wrong price. The static `$3000` value only survives as the fallback for chains with no Chainlink feed (`eth_usd_price_feed == 0x0`, e.g. local `anvil`/devnet). |
| Gas spike between quote and settlement | **mitigated (stall + self-heal; finding B1 / security review F4)** | The quote-time `WITHDRAWAL_FEE_BUFFER_PERCENT` (20%) buffer absorbs typical fluctuation between a user's quote and the batch actually settling. If gas rises enough that a timed-out batch's reprice would still exceed the sum of its withdrawals' committed `max_fee` (the ceiling), the federation does **not** execute the batch over-ceiling and does **not** refund the burned e-cash (the pre-fix behavior refunded the e-cash while leaving the already-signed op live on-chain — a double-pay if it later confirmed). Instead `process_replace_user_op` STALLS: the op stays live and non-superseded at its `(sender, nonce)`, so a late on-chain confirmation still settles it exactly once. `propose_replace_user_ops` re-checks affordability against the fee median every timeout and re-fires the reprice the instant the batch is priced back under the ceiling — self-healing, no operator action needed. Residual cost: because the pool account is one-batch-at-a-time, a stalled batch blocks subsequent withdrawal batches until it reprices or confirms, so a sustained gas spike above a batch's ceiling can leave its withdrawals visibly "stuck" in `Submitted`; a permanent gas-regime shift that never falls back under the ceiling requires operator intervention. |
| EIP-1559 gas sharpening | **follow-up** | The gas-price half of `FeeVote` still uses `provider.get_gas_price()` (legacy-style, buffered 20%) rather than base-fee + priority-fee estimation; a separate refinement, not required for the price-feed work. |
