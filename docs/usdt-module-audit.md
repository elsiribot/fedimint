...
| Gas spike between quote and settlement | **deferred (residual, unchanged by the price feed)** | If gas rises more than `WITHDRAWAL_FEE_BUFFER_PERCENT` (20%) between a user's quote and the batched withdrawal actually settling, the federation eats the difference — a batching-economics question (cap-spend / re-quote / defer-on-spike), independent of how `usdt_per_eth_e6` itself is sourced. |
