...
impl<M: Send + 'static> P2PConnectionSMCommon<M> {
        ...
        loop {
            tokio::select! {
                ...
                () = sleep(METADATA_REFRESH_INTERVAL) => {
                    ...
                    // provide. Refreshing in place rather than re-entering the
                    // connected state means a send in flight is never
                    // cancelled by a metadata refresh.
    ...
    /// non-cancel-safe `send` and silently drop a message that the DKG would
    /// not resend. Only a failure of the connection itself can still lose the
    /// message in flight; the networking layer is unreliable by design.
    async fn send_loop(
        connection: &DynP2PConnection<M>,
        ...
        loop {
            // Both branches are cancel-safe: `recv` on an async channel does
            // not take an item unless it completes.
