pub const FM_DISABLE_META_MODULE_ENV: &str = "FM_DISABLE_META_MODULE";

pub const FM_DATA_DIR_ENV: &str = "FM_DATA_DIR";

pub const FM_BIND_TOKIO_CONSOLE_ENV: &str = "FM_BIND_TOKIO_CONSOLE";

pub const FM_BIND_P2P_ENV: &str = "FM_BIND_P2P";

pub const FM_P2P_URL_ENV: &str = "FM_P2P_URL";

pub const FM_BIND_API_ENV: &str = "FM_BIND_API";

pub const FM_API_URL_ENV: &str = "FM_API_URL";

pub const FM_BIND_UI_ENV: &str = "FM_BIND_UI";

pub const FM_BITCOIN_NETWORK_ENV: &str = "FM_BITCOIN_NETWORK";

pub const FM_BIND_METRICS_ENV: &str = "FM_BIND_METRICS";

pub const FM_PORT_ESPLORA_ENV: &str = "FM_PORT_ESPLORA";

pub const FM_FORCE_API_SECRETS_ENV: &str = "FM_FORCE_API_SECRETS";

pub const FM_BITCOIND_URL_ENV: &str = "FM_BITCOIND_URL";

pub const FM_BITCOIND_URL_PASSWORD_FILE_ENV: &str = "FM_BITCOIND_URL_PASSWORD_FILE";

pub const FM_ESPLORA_URL_ENV: &str = "FM_ESPLORA_URL";

pub const FM_ENABLE_IROH_ENV: &str = "FM_ENABLE_IROH";

pub const FM_ENABLE_API_TOR_ENV: &str = "FM_ENABLE_API_TOR";

pub const FM_TOR_API_SERVICE_NAME_ENV: &str = "FM_TOR_API_SERVICE_NAME";

pub const FM_TOR_API_PORT_ENV: &str = "FM_TOR_API_PORT";

pub const FM_DB_CHECKPOINT_RETENTION_ENV: &str = "FM_DB_CHECKPOINT_RETENTION";

pub const FM_IROH_API_MAX_CONNECTIONS_ENV: &str = "FM_IROH_API_MAX_CONNECTIONS";

pub const FM_BITCOIND_USERNAME_ENV: &str = "FM_BITCOIND_USERNAME";

pub const FM_BITCOIND_PASSWORD_ENV: &str = "FM_BITCOIND_PASSWORD";

// TODO: Eventually Iroh should provide something better than this.
// <https://github.com/n0-computer/iroh/discussions/3212>
pub const FM_IROH_API_MAX_REQUESTS_PER_CONNECTION_ENV: &str =
    "FM_IROH_API_MAX_REQUESTS_PER_CONNECTION";
